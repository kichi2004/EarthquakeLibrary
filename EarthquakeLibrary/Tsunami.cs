...
public static class Tsunami
{
    ...
    /// </summary>
    /// <returns></returns>
    public static async Task<(IEnumerable<ForecastResult>, EpicenterInfo)> GetForecastFromJmaAsync()
        => await GetForecastFromJmaAsync(new Uri(JmaTsunami));
    ...
    /// <returns></returns>
    public static async Task<(IEnumerable<ForecastResult>, EpicenterInfo)> GetForecastFromJmaAsync(string url)
        => await GetForecastFromJmaAsync(new Uri(url));
    ...
    /// <returns></returns>
    public static async Task<(IEnumerable<ForecastResult>, EpicenterInfo)> GetForecastFromJmaAsync(Uri uri) {
        //URLからダウンロード（ソースを取得）
        var source = await LibCore.DownloadString(uri);
        //震源要素を正規表現で取得
        var match =
    ...
    /// <returns></returns>
    public static async Task<IEnumerable<ObservationResult>> GetObservationFromTenkiJp(Uri uri)
    {
        string html = await LibCore.DownloadString(uri);
        ...
        foreach (Match m in matches)
        {
            ...
            var obs = new ObservationResult
            {
                ...
                Max = m.Groups[4].Value == "---"
                    ? null
                    : DateTime.ParseExact(m.Groups[4].Value, "M月d日 H時m分", null),
    ...
    /// <param name="uri"></param>
    /// <returns></returns>
    public static async Task<IEnumerable<ObservationResult>> GetObservationFromTenkiJp(string uri)
        => await GetObservationFromTenkiJp(new Uri(uri));

    /// <summary>
    /// tenki.jpから津波観測情報を取得します。
    /// </summary>
    /// <returns></returns>
    public static async Task<IEnumerable<ObservationResult>> GetObservationFromTenkiJp()
        => await GetObservationFromTenkiJp(new Uri(TenkiTsunami));
    ...
    /// <param name="uri"></param>
    /// <returns></returns>
    public static async Task<IEnumerable<ObservationResult>> GetOffshoreFromTenkiJp(Uri uri) {
        string html = await LibCore.DownloadString(uri);
        var str =
            ...
            foreach (Match m in matches) {
                ...
                var obs = new ObservationResult
                {
                    ...
                    Max = m.Groups[3].Value == "---" ? null :
                        DateTime.ParseExact(m.Groups[3].Value, "M月d日 H時m分", null) ,
    ...
    /// <param name="uri"></param>
    /// <returns></returns>
    public static async Task<IEnumerable<ObservationResult>> GetOffshoreFromTenkiJp(string uri)
        => await GetOffshoreFromTenkiJp(new Uri(uri));

    /// <summary>
    /// tenki.jpから沖合観測情報を取得します。
    /// </summary>
    /// <returns></returns>
    public static async Task<IEnumerable<ObservationResult>> GetOffshoreFromTenkiJp()
        => await GetOffshoreFromTenkiJp(new Uri(TenkiTsunami));
